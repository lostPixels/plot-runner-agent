...
class NextDrawAPITester:
    ...
    def test_pause_resume(self):
        ...
        try:
            # Create a simple test SVG that takes time to plot
            test_svg = '''<svg width="200" height="200" xmlns="http://www.w3.org/2000/svg">
                <path d="M 20 20 L 180 20 L 180 180 L 20 180 Z" stroke="black" stroke-width="2" fill="none"/>
            ...
            # Submit a job
            job_data = {
                "name": "API_Test_Pause_Resume",
                "description": "Test job for pause/resume functionality",
                "svg_content": test_svg,
                "config_overrides": {
                    "speed_pendown": 5,  # Slow speed to give time for pause/resume
                    "report_time": True
                }
            }
            ...
            if response.status_code == 201:
                ...
                if job_id:
                    self.log_test("Submit Job for Pause/Resume", True, f"Job {job_id} submitted")

                    # Wait for job to start
                    time.sleep(3)

                    # Send pause request
                    response = self.session.post(f"{self.base_url}/pause", json={})

                    if response.status_code == 200:
                        self.log_test("Pause Plot", True, "Plot paused successfully")

                        # Check status to confirm it's paused
                        response = self.session.get(f"{self.base_url}/status")
                        if response.status_code == 200:
                            status_data = response.json()
                            plotter_status = status_data.get("plotter", {}).get("status")
                            is_paused = status_data.get("plotter", {}).get("is_paused", False)

                            if plotter_status == "PAUSED" or is_paused:
                                self.log_test("Pause Status Check", True, f"Plotter is paused: {plotter_status}")

                                # Wait a moment before resuming
                                time.sleep(2)

                                # Send resume request
                                response = self.session.post(f"{self.base_url}/resume", json={})

                                if response.status_code == 200:
                                    self.log_test("Resume Plot", True, "Plot resumed successfully")

                                    # Check status to confirm it's plotting again
                                    response = self.session.get(f"{self.base_url}/status")
                                    if response.status_code == 200:
                                        status_data = response.json()
                                        plotter_status = status_data.get("plotter", {}).get("status")
                                        is_plotting = status_data.get("plotter", {}).get("is_plotting", False)

                                        if plotter_status == "PLOTTING" or is_plotting:
                                            self.log_test("Resume Status Check", True, f"Plotter is plotting again: {plotter_status}")

                                            # Cancel the job for cleanup
                                            time.sleep(2)
                                            response = self.session.post(f"{self.base_url}/stop", json={})
                                            if response.status_code == 200:
                                                self.log_test("Stop Job", True, "Job stopped successfully")
                                                return True
                                            else:
                                                self.log_test("Stop Job", False, f"Failed to stop job: HTTP {response.status_code}")
                                        else:
                                            self.log_test("Resume Status Check", False, f"Plotter not plotting after resume: {plotter_status}")
                                    else:
                                        self.log_test("Resume Status Check", False, f"Failed to get status: HTTP {response.status_code}")
                                else:
                                    self.log_test("Resume Plot", False, f"Failed to resume: HTTP {response.status_code}")
                            else:
                                self.log_test("Pause Status Check", False, f"Plotter not paused: {plotter_status}")
                        else:
                            self.log_test("Pause Status Check", False, f"Failed to get status: HTTP {response.status_code}")
                    else:
                        self.log_test("Pause Plot", False, f"Failed to pause: HTTP {response.status_code}")

                    # Make sure to stop the job if it's still running
                    response = self.session.post(f"{self.base_url}/stop", json={})
                else:
                    self.log_test("Submit Job for Pause/Resume", False, "No job ID returned", job_result)
            else:
                self.log_test("Submit Job for Pause/Resume", False, f"HTTP {response.status_code}")

        except Exception as e:
            self.log_test("Pause/Resume Test", False, f"Request failed: {str(e)}")
            # Try to stop any running job
            try:
                self.session.post(f"{self.base_url}/stop", json={})
            except:
                pass
